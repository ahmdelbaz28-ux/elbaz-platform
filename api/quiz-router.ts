...
export const quizRouter = createRouter({
  ...
  submit: authedQuery
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (recentAttempts.length > 0 && recentAttempts[0].quizScore >= 70) {
        throw new TRPCError({
      ...
      const results = input.answers.map((ans) => {
        ...
        return {
          ...
          correctAnswer: isCorrect ? question?.correctOptionIndex : undefined,
          explanation: isCorrect ? question?.explanationEn : undefined,
        };
      });
