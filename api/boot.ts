...
app.get("/api/health", async (c) => {
  ...
  return c.json({ status: dbStatus === "ok" ? "ok" : "degraded", ts: new Date().toISOString(), db: dbStatus, dbError: dbError ? dbError.substring(0, 200) : undefined });
});
...
app.use("*", async (c, next) => {
  ...
  if (!c.req.path.startsWith("/api")) {
    ...
    c.header("Permissions-Policy", "camera=(), microphone=(), geolocation=(), payment=(), usb=(), magnetometer=()");
