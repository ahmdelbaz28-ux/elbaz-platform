...
/** Redis key storing the global notification event list (JSON array). */
const NOTIFICATIONS_KEY = "notifications:global";
 ...
 */
async function readNotifications(): Promise<NotificationEvent[] | null> {
  try {
    const cache = getCache();
    const raw = await cache.get<NotificationEvent[]>(NOTIFICATIONS_KEY);
    return raw;
  } catch {
...
export const notificationPollRouter = createRouter({
  poll: authedQuery
    ...
    .query(async ({ ctx, input }) => {
      ...
      // ── Read notification events from cache ───────────────────────────
      const allEvents = await readNotifications();
...
/**
 * Publish a notification event to the global notification stream.
 *
...
export async function pushNotificationEvent(
  ...
  try {
    ...
    // Read existing events, prepend the new one, cap at 200 to prevent
    // unbounded growth in the cache key.
    const existing = await cache.get<NotificationEvent[]>(NOTIFICATIONS_KEY);
    const updated = [event, ...(existing || [])].slice(0, 200);

    await cache.set(NOTIFICATIONS_KEY, updated, 86400); // 24 hours TTL
  } catch {
