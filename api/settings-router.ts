  ...
  upsert: adminQuery
    ...
    .mutation(async ({ input }) => {
      ...
      const [existing] = await db
        .select()
        .from(siteSettings)
        .where(and(eq(siteSettings.section, input.section), eq(siteSettings.key, input.key)))
        .limit(1);

      if (existing) {
        await db.update(siteSettings).set({ value: input.value, type: input.type }).where(eq(siteSettings.id, existing.id));
      } else {
    ...
    .mutation(async ({ input }) => {
      ...
      for (const s of input.settings) {
        const [existing] = await db
          .select()
          .from(siteSettings)
          .where(and(eq(siteSettings.section, input.section), eq(siteSettings.key, s.key)))
          .limit(1);
        if (existing) {
          await db.update(siteSettings).set({ value: s.value, type: s.type }).where(eq(siteSettings.id, existing.id));
        } else {
          await db.insert(siteSettings).values({ ...s, section: input.section });
        }
      }
      return { success: true };
    }),
