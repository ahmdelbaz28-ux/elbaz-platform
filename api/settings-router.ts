...
import { invalidateSettingsCache, getCache, cacheKeys, CACHE_TTL } from "./lib/cache";
...
export const settingsRouter = createRouter({
  ...
  getSection: publicQuery
    ...
    .query(async ({ input }) => {
      ...
      const cacheKey = cacheKeys.settingsSection(input.section);
      ...
      await cache.set(cacheKey, result, CACHE_TTL.SETTINGS_SECTION);
      return result;
    }),
  ...
  // Seed default themes (including Detego)
  seedThemes: adminQuery.mutation(async () => {
    const db = getDb();
    const [existing] = await db.select({ id: themes.id }).from(themes).limit(1);
    if (existing) return { success: true, message: "Themes already exist" };

    await db.insert(themes).values([
      {
        name: "Elbaz Dark (Default)",
        slug: "elbaz-dark",
        primaryColor: "#06b6d4",
      ...
      {
        name: "Detego Professional",
        slug: "detego",
        primaryColor: "#0ea5e9", // Clean blue
        secondaryColor: "#3b82f6",
        accentColor: "#f59e0b",
        bgColor: "#0f172a", // Very dark slate (modern engineering apps)
        cardBgColor: "#1e293b",
        textColor: "#f8fafc",
        mutedTextColor: "#94a3b8",
        borderColor: "#334155",
  ...
  getActivePromotions: publicQuery.query(async () => {
    const cache = getCache();
    const cached = await cache.get(cacheKeys.activePromotions());
    if (cached) return cached;
    ...
    await cache.set(cacheKeys.activePromotions(), result, CACHE_TTL.ACTIVE_PROMOTIONS);
    return result;
  }),
