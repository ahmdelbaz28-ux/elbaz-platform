...
export async function initiateEmailVerification(userId: number): Promise<{
  ...
  const [user] = await db
    .select({ id: users.id, email: users.email, name: users.name, username: users.username, emailVerifiedAt: users.emailVerifiedAt })
    .from(users)
    .where(eq(users.id, userId))
    .limit(1);
...
export async function completeEmailVerification(
  ...
  const tokenHash = crypto.createHash("sha256").update(token).digest("hex");
  if (!user.emailVerificationToken || tokenHash !== user.emailVerificationToken) {
    return { success: false, message: "Invalid verification link." };
  }
