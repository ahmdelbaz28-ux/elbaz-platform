...
import { Clock, Star, Users, Zap, Lock, ChevronRight } from "lucide-react";
...
export default function CourseCard({ course }: CourseCardProps) {
  ...
  return (
    <Link to={`/courses/${course.slug}`} className="group block outline-none focus-visible:ring-2 focus-visible:ring-[#06b6d4] rounded-2xl">
      <article
        className="relative overflow-hidden rounded-2xl border border-[#1e2d3d] bg-[#0d1420] transition-all duration-300
          group-hover:border-[rgba(6,182,212,0.4)] group-hover:shadow-[0_20px_60px_rgba(6,182,212,0.08)]
      ...
      >
        ...
        <div className="relative h-48 overflow-hidden">
          ...
          <div className="absolute bottom-3 right-3 flex items-center gap-1 rounded-lg bg-[rgba(10,14,23,0.75)] px-2 py-1 backdrop-blur-md">
        ...
        <div className="p-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex h-8 w-8 items-center justify-center rounded-full border border-[#1e2d3d] bg-[#111827] transition-all duration-200 group-hover:border-[#06b6d4] group-hover:bg-[rgba(6,182,212,0.1)]">
              <ChevronRight className="h-4 w-4 text-[#475569] transition-colors group-hover:text-[#06b6d4]" />
            </div>
          </div>
        </div>

        {/* Shimmer line on hover */}
        <div className="absolute bottom-0 left-0 h-[2px] w-0 bg-gradient-to-r from-[#06b6d4] to-[#0891b2] transition-all duration-500 group-hover:w-full" />
      </article>
    </Link>
  );
}
