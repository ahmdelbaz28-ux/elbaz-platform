...
function TicketThread({ ticketId, initialMessage }: { ticketId: number, initialMessage: string }) {
  ...
  if (isLoading) {
    return <div className="py-4 text-center text-sm text-[#94a3b8]">{t("loading")}</div>;
  }
    ...
    <div className="mt-4 border-t border-[#1f2d44] pt-4">
      <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
        ...
        <div className="flex flex-col items-start">
          <div className="rounded-lg bg-[#1f2d44] px-4 py-2 text-sm text-[#f0f4f8]">
            {initialMessage}
          </div>
        </div>
        {/* Replies */}
        {ticketDetail?.replies?.map((reply: any) => (
          <div key={reply.id} className={`flex flex-col ${reply.isAdminReply ? "items-start" : "items-end"}`}>
            <span className="mb-1 text-[10px] text-[#64748b]">
              {reply.isAdminReply ? "Support Team" : "You"}
            </span>
            <div className={`rounded-lg px-4 py-2 text-sm ${reply.isAdminReply ? "bg-[rgba(6,182,212,0.15)] text-[#22d3ee] border border-[#06b6d4]/20" : "bg-[#1f2d44] text-[#f0f4f8]"}`}>
              {reply.message}
            </div>
          </div>
        ))}
...
export default function Support() {
  ...
  const { data: ticketsData, isLoading } = trpc.support.list.useQuery<TicketsResponse>(undefined, {
