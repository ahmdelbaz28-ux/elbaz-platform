...
interface SettingItem {
  ...
  value: string;
  section: string;
  type?: "number" | "json" | "text" | "richtext" | "image" | "url" | "color";
}
...
export default function Admin() {
  ...
  const handleCmsSave = () => {
   ...
   const seedThemes = trpc.settings.seedThemes.useMutation({
  ...
  const handleSavePromo = () => {
    ...
    if (editingPromo.id) {
      updatePromo.mutate({
        id: Number(editingPromo.id),
        code: editingPromo.code,
        discount: editingPromo.discount,
        type: editingPromo.type,
        maxUses: editingPromo.maxUses,
        expiresAt: editingPromo.validUntil || undefined,
        validFrom: editingPromo.validFrom || undefined,
        isActive: editingPromo.status === "active" ? true : false,
      });
    } else {
      createPromo.mutate({
        code: editingPromo.code,
        discount: editingPromo.discount ?? 10,
        type: editingPromo.type ?? "percentage",
        maxUses: editingPromo.maxUses ?? 100,
        expiresAt: editingPromo.validUntil || undefined,
        validFrom: editingPromo.validFrom || undefined,
      });
    }
  ...
  const { data: contactData, isLoading: contactLoading } = trpc.settings.getSection.useQuery(
    { section: "contact" },
    { enabled: !!isAdmin },
  );
  const [contactValues, setContactValues] = useState<Record<string, string>>({});
  const contactSave = trpc.settings.bulkUpsert.useMutation({
    onSuccess: () => {
      utils.settings.getSection.invalidate({ section: "contact" });
      utils.settings.getAll.invalidate();
    },
    onError: (err) => console.warn("[Admin] contactSave failed:", err.message),
  });
  ...
  useEffect(() => {
    if (contactData) {
      if (Array.isArray(contactData)) {
        const map: Record<string, string> = {};
        (contactData as SettingItem[]).forEach((s) => { map[s.key] = s.value; });
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setContactValues(map);
      } else {
         
        setContactValues(contactData as Record<string, string>);
      }
    }
  }, [contactData]);

  const handleContactSave = () => {
    type SettingType = "number" | "json" | "text" | "richtext" | "image" | "url" | "color";
    const settings = Object.entries(contactValues).map<{ key: string; value: string; type: SettingType }>(([key, value]) => ({
      key,
      value,
      type: (key.includes("Url") ? "url" : "text") as SettingType,
    }));
    contactSave.mutate({ section: "contact", settings });
  };
      ...
      <div className="mx-auto max-w-7xl px-4 pb-20 lg:px-6">
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {[
            ...
            { icon: <DollarSign className="h-5 w-5" />, label: t("totalRevenue"), value: `${stats?.totalRevenue || 0} EGP`, color: "#f59e0b" },
        ...
        <Tabs defaultValue="users" className="space-y-6">
          <TabsList className="flex-wrap border border-[#1f2d44] bg-[#111827]">
            ...
            <TabsTrigger value="contact" className="data-[state=active]:bg-[#06b6d4] data-[state=active]:text-[#0a0e17]">
              <Phone className="mr-2 h-4 w-4" /> {isRTL ? "معلومات التواصل" : "Contact"}
            </TabsTrigger>
          ...
          <TabsContent value="contact">
            <div className="rounded-xl border border-[#1f2d44] bg-[#111827] p-6">
              ...
              {contactLoading ? renderLoader() : (
                <div className="space-y-8">
                  {/* ─── WhatsApp & Phone ─── */}
                  <div>
                    <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold text-[#06b6d4]">
                      <span className="inline-block h-2 w-2 rounded-full bg-[#25D366]" />
                      {isRTL ? "واتساب والهاتف" : "WhatsApp & Phone"}
                    </h3>
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="grid gap-1.5">
                        ...
                        <Input
                          value={contactValues.whatsappNumber || ""}
                          onChange={(e) => setContactValues((p) => ({ ...p, whatsappNumber: e.target.value }))}
                          placeholder="201061857305"
                      ...
                      <div className="grid gap-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) => setContactValues((p) => ({ ...p, phone: e.target.value }))}
                          placeholder="01061857305"
                          className="border-[#1f2d44] bg-[#0a0e17] text-[#f0f4f8]"
                          dir="ltr"
                        />
                      </div>
                  ...
                  <div>
                    ...
                    <div className="grid gap-1.5 sm:max-w-md">
                      ...
                      <Input
                        ...
                        onChange={(e) => setContactValues((p) => ({ ...p, email: e.target.value }))}
                  ...
                  <div>
                    <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold text-[#06b6d4]">
                      <span className="inline-block h-2 w-2 rounded-full bg-[#25D366]" />
                      {isRTL ? "رسائل واتساب" : "WhatsApp Messages"}
                    </h3>
                    <div className="grid gap-4">
                      <div className="grid gap-1.5">
                        <Label className="text-sm text-[#94a3b8]">
                          {isRTL ? "الرسالة (إنجليزي)" : "Default Message (English)"}
                        </Label>
                        <Textarea
                          value={contactValues.whatsappMessageEn || ""}
                          onChange={(e) => setContactValues((p) => ({ ...p, whatsappMessageEn: e.target.value }))}
                          className="min-h-20 border-[#1f2d44] bg-[#0a0e17] text-[#f0f4f8]"
                          dir="ltr"
                        />
                      </div>
                      <div className="grid gap-1.5">
                        <Label className="text-sm text-[#94a3b8]">
                          {isRTL ? "الرسالة (عربي)" : "Default Message (Arabic)"}
                        </Label>
                        <Textarea
                          value={contactValues.whatsappMessageAr || ""}
                          onChange={(e) => setContactValues((p) => ({ ...p, whatsappMessageAr: e.target.value }))}
                          className="min-h-20 border-[#1f2d44] bg-[#0a0e17] text-[#f0f4f8]"
                          dir="rtl"
                        />
                      </div>
                  ...
                  <div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      {([
                        { key: "youtubeUrl", labelEn: "YouTube URL", labelAr: "رابط يوتيوب" },
                        { key: "linkedinUrl", labelEn: "LinkedIn URL", labelAr: "رابط لينكد إن" },
                        { key: "facebookUrl", labelEn: "Facebook URL", labelAr: "رابط فيسبوك" },
                        { key: "instagramUrl", labelEn: "Instagram URL", labelAr: "رابط إنستاجرام" },
                        { key: "tiktokUrl", labelEn: "TikTok URL", labelAr: "رابط تيك توك" },
                        { key: "twitterUrl", labelEn: "Twitter / X URL", labelAr: "رابط تويتر / X" },
                        ...
                        <div key={key} className="grid gap-1.5">
                          ...
                          <div className="flex items-center gap-2">
                            <Input
                              ...
                              dir="ltr"
                            />
                            {contactValues[key] && contactValues[key] !== "#" && (
                              <a
                                href={contactValues[key]}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-[#1f2d44] text-[#94a3b8] transition-colors hover:border-[#06b6d4] hover:text-[#06b6d4]"
                  ...
                  <div>
                    ...
                    <div className="grid gap-1.5 sm:max-w-md">
                      ...
                      <div className="flex items-center gap-2">
                        <Input
                          value={contactValues.websiteUrl || ""}
                          onChange={(e) => setContactValues((p) => ({ ...p, websiteUrl: e.target.value }))}
                          placeholder="https://..."
                          className="flex-1 border-[#1f2d44] bg-[#0a0e17] text-[#f0f4f8]"
                          dir="ltr"
                        />
                        {contactValues.websiteUrl && (
                          <a
                            href={contactValues.websiteUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-[#1f2d44] text-[#94a3b8] transition-colors hover:border-[#06b6d4] hover:text-[#06b6d4]"
                            aria-label={isRTL ? "فتح الرابط" : "Open link"}
                  ...
                  <div className="flex justify-end pt-4 border-t border-[#1f2d44]">
                    <Button
                      ...
                      disabled={contactSave.isPending}
                      className="bg-[#06b6d4] text-[#0a0e17] hover:bg-[#22d3ee]"
                    >
                      {contactSave.isPending ? (
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                {(!themes || themes.length === 0) && (
                  <Button
                    onClick={() => seedThemes.mutate()}
                    variant="outline"
                    className="border-[#1f2d44] hover:bg-[#1f2d44]"
                    disabled={seedThemes.isPending}
                  >
                    {isRTL ? "تحميل الثيمات الافتراضية" : "Load Default Themes"}
                  </Button>
                )}
              </div>
                        ...
                        <div className="mt-3 flex gap-2">
                          ...
                          {!theme.isActive && (
                            <Button
                              size="sm"
                              variant="ghost"
                               onClick={() => activateTheme.mutate({ id: Number(theme.id) })}
            ...
            <Dialog open={!!deleteConfirmId} onOpenChange={() => setDeleteConfirmId(null)}>
              <DialogContent className="border-[#1f2d44] bg-[#111827] sm:max-w-md">
                ...
                <DialogFooter>
                  ...
                  <Button
                    onClick={() => deleteConfirmId && deleteTheme.mutate({ id: Number(deleteConfirmId) })}
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="grid gap-1.5">
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditingPromo((p) => p ? { ...p, validFrom: e.target.value } : p)}
                        className="border-[#1f2d44] bg-[#0a0e17] text-[#f0f4f8]"
                        dir="ltr"
                      />
                    </div>
                    <div className="grid gap-1.5">
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditingPromo((p) => p ? { ...p, validUntil: e.target.value } : p)}
                        className="border-[#1f2d44] bg-[#0a0e17] text-[#f0f4f8]"
                        dir="ltr"
                      />
                    </div>
              ...
              <Dialog open={!!deletePromoId} onOpenChange={() => setDeletePromoId(null)}>
                <DialogContent className="border-[#1f2d44] bg-[#111827] sm:max-w-md">
                  ...
                  <DialogFooter>
                    ...
                    <Button
                       onClick={() => deletePromoId && deletePromo.mutate({ id: Number(deletePromoId) })}
              ...
              <Dialog open={!!deletePromotionId} onOpenChange={() => setDeletePromotionId(null)}>
                <DialogContent className="border-[#1f2d44] bg-[#111827] sm:max-w-md">
                  ...
                  <DialogFooter>
                    ...
                    <Button
                       onClick={() => deletePromotionId && deletePromotion.mutate({ id: Number(deletePromotionId) })}
