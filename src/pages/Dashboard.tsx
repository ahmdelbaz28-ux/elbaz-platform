...
export default function Dashboard() {
  ...
  const totalSpent = payments?.reduce((sum, p) => sum + (parseFloat(String(p.amount)) || 0), 0) || 0;
      ...
      <div className="mx-auto max-w-7xl px-4 pb-20 lg:px-6">
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { icon: <Clock className="h-5 w-5" />, label: lang === "en" ? "Total Spent" : "إجمالي المدفوع", value: `${totalSpent.toLocaleString()} ${lang === "ar" ? "ج.م" : "EGP"}` },
            { icon: <TrendingUp className="h-5 w-5" />, label: lang === "en" ? "Payments" : "المدفوعات", value: payments?.length || 0 },
          ].map((stat, i) => (
