...
export function useChatFabAnimation({
  ...
  const createSparkle = useCallback(() => {
    ...
    let container = fab.parentElement?.querySelector(
      ".sparkle-container"
    ) as HTMLElement | null;
    ...
    if (!container) {
      ...
      // Make the FAB parent relative for positioning
      const fabWrapper = fab.parentElement;
      if (fabWrapper) {
        const currentPos = getComputedStyle(fabWrapper).position;
        if (currentPos === "static") {
          fabWrapper.style.position = "relative";
        }
        fabWrapper.appendChild(container);
      }
    }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const fab = fabRef.current;
      if (fab) {
        const container = fab.parentElement?.querySelector(".sparkle-container");
